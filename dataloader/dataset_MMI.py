        ...
        for seg_ind in indices:
            ...
            for i in range(self.duration):
                try:
                    ...
                    print("something went wrong",record.path,len(video_frames_path),indices,p)
...
def train_data_loader(data_set):
    ...
    train_transforms = torchvision.transforms.Compose([GroupRandomSizedCrop(image_size),
                                                       ...
                                                       Stack(),
                                                       ToTorchFormatTensor()])
    train_data = VideoDataset(list_file="./annotation/MMI_set_"+str(data_set)+"_train.txt",
                              num_segments=8,
                              duration=2,
                              mode='train',
...
def test_data_loader(data_set):
    ...
    test_transform = torchvision.transforms.Compose([GroupResize(image_size),
                                                     Stack(),
                                                     ToTorchFormatTensor()])
    test_data = VideoDataset(list_file="./annotation/MMI_set_"+str(data_set)+"_test.txt",
                             num_segments=8,
                             duration=2,
                             mode='test',
